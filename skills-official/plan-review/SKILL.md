---
name: plan-review
description: Create implementation plan, review it with iterative-review, and append tasks to tasks.yml. Use when asked to "plan", "create a plan for", "break down a feature into tasks", or "add tasks to tasks.yml". Triggers on phrases like "plan this out", "make a task plan", "review my plan".
...
1. Task name path traversal check:
   - Run: `realpath -m "<task-name>"` and confirm result stays within `git rev-parse --show-toplevel`
   - Reject if contains control characters or null bytes
2. Perspectives command injection check:
   - Split by comma, validate each token against allowed list
   - Reject if any token contains shell metacharacters: ; | & $ ` ( ) < > \ " '
   - Pass only validated perspective names to /iterative-review
3. Temporary file security (via Bash tool):
   - Create with: `mktemp` then `chmod 600 "$TMPFILE"`
   - Register cleanup: `trap 'rm -f "$TMPFILE"' EXIT INT TERM`
...
Prerequisite: /iterative-review skill must be installed (`~/.claude/skills/iterative-review/SKILL.md`).
If not found: report "/iterative-review command not available" and exit.
...
tasks_updater.py auto-initializes tasks.yml if missing (no manual creation needed).
...
Implementation: `${CLAUDE_SKILL_DIR}/tasks_updater.py`

Run via Bash tool:
```
python3 "${CLAUDE_SKILL_DIR}/tasks_updater.py" <tasks_json_file>
```

Input: JSON file with array of task objects (goal, priority, effort, acceptance_criteria).
Output: Appends to tasks.yml; prints "Added N task(s) to tasks.yml" on success.
On failure: non-zero exit code with error message (no stack traces exposed).
...
|----------|-----------|--------|
| User error | Task name missing, invalid arguments | Report allowed values and exit |
| Security error | YAML injection detected, path traversal | Report "Invalid input detected" and exit |
| Dependency error | /iterative-review not available, PyYAML missing | Report missing dependency with install instruction |
| System error | tasks.yml write failure, plan.md creation failure | Save fallback file, report recovery path |
